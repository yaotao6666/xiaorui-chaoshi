...
- 当前版本不启用在线支付、微信支付回调、分账与子商户配置。
- C 端创建订单后直接进入“待后续支付接入”状态，不返回 `pay_params`。
- 后续如接入新的支付方案，单独以新文档和新章节定义，不复用本文件中的旧服务商支付描述。
...
```
用户扫描商家二维码或进入小程序壳 → 完成登录 → 跳转 H5 店铺首页 → 浏览商品 → 选择商品加入购物车 → 确认订单信息 → 选择已开启的下单方式（配送 / 堂食 / 自提）→ 创建订单 → 进入订单列表
```
...
| ---- | ------------------- | --- |
| 创建订单 | 用户下单                | P0  |
| 创建后状态 | 当前版本不启用在线支付，创建订单后进入待后续支付接入状态 | P0  |
| 订单列表 | 商家查看订单列表            | P0  |
| 订单详情 | 查看订单详细信息            | P0  |
...
> **说明**：历史版本中此处曾返回微信支付参数；当前版本下单接口已不再返回 `pay_params`。
...
| 功能   | 描述                 | 对接API                                    |
| ---- | ------------------ | ---------------------------------------- |
| 创建订单 | 提交订单，进入待后续支付接入状态 | POST /api/v1/store/{merchant\_id}/orders |
| 历史支付回调 | 已归档，不再作为当前测试范围   | - |
